...
interface PendingDeposit {
  ...
  description: string;
  reference: string;
  metadata: { method?: string; notes?: string };
  created_at: string;
...
export default function AdminDeposits() {
  ...
  const loadDeposits = useCallback(async () => {
    ...
    if (error) {
      ...
      setDeposits((data as unknown as PendingDeposit[]) || []);
    }
    setLoading(false);
  }, []);
  ...
  const approve = async (deposit: PendingDeposit) => {
    ...
    const { error: txnErr } = await supabase
      .from('transactions')
      .update({ status: 'completed', description: deposit.description.replace('Pending', 'Completed') })
      .eq('id', deposit.id);
    if (txnErr) {
    ...
    toast.success(`Deposit ${deposit.reference} approved and balance credited`);
    setSelected(null);
    await loadDeposits();
    setActionLoading(null);
  };
  ...
  const reject = async (deposit: PendingDeposit) => {
    setActionLoading(deposit.id);
    const { error } = await supabase.from('transactions').update({ status: 'failed' }).eq('id', deposit.id);
    if (error) toast.error('Failed to reject deposit');
    else {
      toast.success(`Deposit ${deposit.reference} rejected`);
      setSelected(null);
      await loadDeposits();
    }
    setActionLoading(null);
  ...
  const filtered = deposits.filter((d) => {
    ...
    const p = d.account?.profiles;
    return (
      d.reference.toLowerCase().includes(q) ||
      d.account?.account_number.toLowerCase().includes(q) ||
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground">Pending Deposits</h1>
          <p className="text-muted-foreground text-sm mt-1">Review and approve user deposit requests</p>
        </div>
          ...
          <table className="w-full whitespace-nowrap">
            ...
            <tbody>
              {loading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <tr><td colSpan={7} className="px-6 py-12 text-center text-muted-foreground">No pending deposits</td></tr>
              ) : filtered.map((d) => {
                const p = d.account?.profiles;
                return (
                  <tr key={d.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-muted-foreground font-mono">{d.account?.account_number}</td>
                    <td className="px-6 py-4 text-sm font-semibold text-green-400">{d.currency} {d.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                    <td className="px-6 py-4 text-sm text-muted-foreground capitalize">{d.metadata?.method?.replace('_', ' ') || '---'}</td>
                    <td className="px-6 py-4 text-sm text-muted-foreground font-mono">{d.reference}</td>
                    <td className="px-6 py-4 text-sm text-muted-foreground">{new Date(d.created_at).toLocaleDateString()}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Button size="sm" className="bg-primary text-primary-foreground hover:bg-primary/90 h-8 px-2 text-xs" onClick={() => setSelected(d)} disabled={actionLoading === d.id}>
                          <DollarSign className="w-3 h-3 mr-1" />Review
                        </Button>
                      </div>
                    </td>
                  </tr>
            ...
            <div className="space-y-4 mt-2">
              <div className="glass-card rounded-xl p-4 border border-border text-center">
                ...
                <div className="text-3xl font-extrabold text-green-400 mt-1">
                  {selected.currency} {selected.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Notes</span><span className="font-medium">{selected.metadata?.notes || '---'}</span></div>
